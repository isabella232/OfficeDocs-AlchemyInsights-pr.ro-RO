---
title: Modificați dimensiunea fontului sau a fontului în Outlook pe web
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000013"
- "1999"
ms.openlocfilehash: 266af20d0c47f328685b190562521227e01635f6
ms.sourcegitcommit: 631cbb5f03e5371f0995e976536d24e9d13746c3
ms.translationtype: MT
ms.contentlocale: ro-RO
ms.lasthandoff: 04/22/2020
ms.locfileid: "43764812"
---
# <a name="change-font-or-font-size-in-outlook-on-the-web"></a>Modificați dimensiunea fontului sau a fontului în Outlook pe web

Când citiți e-mailul, puteți utiliza funcția de zoom a browserului pentru a mări sau micșora textul de pe ecran.
  
Pentru a modifica dimensiunea fontului sau fontului în timp ce compuneți un mesaj:
  
1. În partea de jos a panoului de compunere, selectați <img src='data:image/png;base64,' />
 **Font** sau <img src='data:image/png;base64,' /> **Dimensiune font**.
    
2. Alegeți fontul sau dimensiunea fontului pe care doriți să o utilizați.
    
Dacă nu vedeți meniul de opțiuni de formatare din partea de jos a panoului de compunere, selectați <img src='data:image/png;base64,' /> **Opțiuni de formatare**.
  
Pentru mai multe informații, consultați [Modificarea dimensiunii fontului în Outlook pe web](https://support.office.com/article/43a2137f-8c3c-46df-af4a-73a12c9bb86e).